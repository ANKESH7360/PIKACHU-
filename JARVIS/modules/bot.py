...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        ...
        await jarvis.edit(f"[PIKACHU 𝐈𝐒 𝐑𝐄𝐀𝐃𝐘 𝐓𝐎  ](https://t.me/R_I_S_I_NG)[𝐅𝐔𝐂𝐊 𝐇𝐀𝐓𝐄𝐑𝐒 🥀](https://t.me/R_I_S_I_NG)🤖\n» `{mp} ᴍꜱ`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __Pikachu Ka Ek Beta Aur Add Ho rha hai..__")
        target = ""
        ...
        if str(target) in sudousers:
            await ok.edit(f"YE BHI pikachu KA HI BACHA HAI.. !!")
        else:
            if len(sudousers) > 0:
        ...
        await event.reply("» BSDK SIRF pikachu SUDO DE SKTA HAI...")
...
async def removesudo(event):
    if event.sender_id == OWNER_ID:
        ...
        await event.reply("Only pikachu can remove sudo users.")
...
async def show_sudo_users(event):
    if event.sender_id == OWNER_ID:
        sudo_users_list = "pikachu Ke Bache hai ye:\n"
